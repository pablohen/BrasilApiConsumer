...
public class CepController(IBrasilApi brasilApi, ILogger<CepController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> GetCepV1(
        [Required]
        [RegularExpression(
            @"^\d{5}-?\d{3}$",
    ...
    )
    {
        ...
        _logger.LogInformation("Fetching address for CEP (v1): {Cep}", cep);
        var response = await _brasilApi.GetCepV1Async(cep);
        ...
        _logger.LogInformation("Successfully retrieved address for CEP (v1): {Cep}", cep);
        return Ok(response);
    }
